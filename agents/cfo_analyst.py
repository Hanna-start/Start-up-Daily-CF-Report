...
def money(value: float) -> str:
    """Format an amount in thousands of KRW (KRW '000) with a K suffix."""
    thousands = value / 1000
    return f"(KRW {abs(thousands):,.0f}K)" if value < 0 else f"KRW {thousands:,.0f}K"
...
class CFOAnalystAgent:
    ...
    def analyze_and_report(self, forecast: ForecastResult, validation: NormalizationResult, patterns: list, output_path: str | Path = "output/cfo_control_tower.html", briefing: str | None = None) -> str:
        ...
        daily_json = json.dumps([{"date": r.Date.strftime("%m-%d"), "scenario": r.Scenario, "cash": round(r.EndingCash / 1000)} for r in forecast.daily.itertuples()], ensure_ascii=False)
        monthly_json = json.dumps([
            {"month": r.Month, "scenario": r.Scenario, "inflow": round(r.Inflow / 1000), "outflow": round(r.Outflow / 1000), "cash": round(r.EndingCash / 1000)}
            for r in forecast.monthly.itertuples()
        ], ensure_ascii=False)

        content = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Start-up Daily CF Report</title>
<style>
...
.account-date{{display:block;color:var(--muted);font-weight:400;margin-top:3px}}.reserve-head .eyebrow{{display:block;margin-bottom:5px}}.total-row td{{border-top:2px solid var(--ink);border-bottom:0;font-size:15px}}.badge.ok{{background:#e8f7f2;color:#08775a}}.chart-tools{{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:12px;flex-wrap:wrap}}.scenario-switch{{display:flex;background:#edf1f7;padding:3px;border-radius:9px}}.scenario-switch button{{border:0;background:transparent;color:var(--muted);padding:7px 13px;border-radius:7px;font-weight:700;cursor:pointer}}.scenario-switch button.active{{background:#fff;color:var(--ink);box-shadow:0 1px 5px #14233d18}}.legend{{display:flex;gap:16px;color:var(--muted);font-size:12px}}.legend i{{display:inline-block;width:10px;height:10px;border-radius:3px;margin-right:5px}}.legend .in{{background:#10a47b}}.legend .out{{background:#ef8b82}}.legend .cashline{{background:#2864dc;height:3px;border-radius:0;vertical-align:3px}}.chart-note{{color:var(--muted);font-size:12px;margin:8px 0 0}}
</style></head><body>
<header><div><h1>Start-up Daily CF Report</h1><p>As of {forecast.as_of_date} · Forecast confidence: {forecast.confidence} · Unit: KRW '000 (thousands of KRW)</p></div><nav><button class="active" data-page="daily">Daily Cash Position</button><button data-page="monthly">12-Month Outlook</button></nav></header><main>
